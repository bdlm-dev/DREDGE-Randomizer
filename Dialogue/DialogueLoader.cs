...
public class DialogueLoader
{
    ...
    /// <summary>
    /// Replace an instruction in a node at a specified index and with operands.
    /// </summary>
    /// <param name="nodeID">The ID of the node.</param>
    /// <param name="index">The index to replace the instruction at.</param>
    /// <param name="opCode">The opcode to insert. See <see href="https://github.com/YarnSpinnerTool/YarnSpinner/blob/main/YarnSpinner/yarn_spinner.proto"/> for a list of opcodes.</param>
    /// <param name="operands">Supported types: string, bool, float, int</param>
    public static void ReplaceInstruction(string nodeID, int index, Yarn.Instruction.Types.OpCode opCode, params object[] operands)
    {
        var instruction = CreateInstruction(opCode, operands);
        ...
        program.Nodes[nodeID].Instructions[index] = instruction;
    }

    /// <summary>
    /// Replace an instruction in a node by target opcode and operands.
    /// </summary>
    /// <param name="nodeID">The ID of the node.</param>
    /// <param name="index">The target instruction to replace.</param>
    /// <param name="opCode">The opcode to insert. See <see href="https://github.com/YarnSpinnerTool/YarnSpinner/blob/main/YarnSpinner/yarn_spinner.proto"/> for a list of opcodes.</param>
    /// <param name="operands">Supported types: string, bool, float, int</param>
    public static void ReplaceInstruction(string nodeID, Instruction target, Yarn.Instruction.Types.OpCode opCode, params object[] operands)
    {
        var newInstruction = CreateInstruction(opCode, operands);

        DredgeDialogueRunner runner = GameManager.Instance.DialogueRunner;
        Program program = Traverse.Create(runner.Dialogue).Field("program").GetValue<Program>();

        for (int i = 0; i < program.Nodes[nodeID].Instructions.Count(); i++) {
            Instruction instruction = program.Nodes[nodeID].Instructions[i];
            if (instruction.Opcode == target.Opcode)
            {
                if (instruction.Operands.Equals(target.Operands))
                {
                    program.Nodes[nodeID].Instructions[i] = newInstruction;
                    break;
                }
            }
        }
    ...
    public static Instruction CreateInstruction(Instruction.Types.OpCode opCode, params object[] operands)
    {
